...
export function readBoardFilters(params: Record<string, string | string[] | undefined>) {
  ...
  // values reach a raw `::date` cast — a forged URL was a 500, not a dropped
  // filter (#514). `calendarDay` is the one round-trip check (import-free, so
  // safe in this client-imported file), and it also drops year 0000 (U43).
  const date = (key: string) => calendarDay(get(key)) ?? undefined;
