...
export function buildTools(actor: AssistantActor): AssistantTool[] {
  ...
  tools.push(
    ...
    },
    {
      name: 'cash_flow',
      description:
        'Движение денег за период (from/to, формат YYYY-MM-DD): поступления клиентов, возвраты клиентам (clientRefunds), партнёрские приход/расход, расходы по категориям. Единственный надёжный источник «сколько денег пришло/ушло».',
      input_schema: {
        type: 'object',
        properties: {
