 ...
 * never the server's. Zero imports: the page and a unit test both call it.
 */
export function spendDateOf(sentAt: Date, timeZone: string): string {
  try {
    ...
    // An unknown zone name must not white-page the expense book.
    return sentAt.toISOString().slice(0, 10);
  }
}
