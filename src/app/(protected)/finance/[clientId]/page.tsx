  ...
  return (
    <div className="mx-auto max-w-lg space-y-4 md:max-w-2xl">
      ...
      <div className="card flex items-baseline gap-2">
        ...
        <span
          className={`font-mono text-2xl font-extrabold ${balance > 0.009 ? 'text-red-700' : 'text-green-700'}`}
        >
          ${balance.toFixed(2)}
        </span>
        {balance > 0.009 && <span className="text-sm font-semibold text-red-700">{t('debtor')}</span>}
      </div>
      ...
      <div className="card space-y-1 !p-3">
        ...
        {ledger.length === 0 && <p className="text-sm text-gray-500">{t('empty')}</p>}
        {ledger.map(({ tx, createdByName, batchCode }) => (
          <div
            key={tx.id}
            className={`border-b border-gray-100 py-2 text-sm last:border-0 ${tx.voidedAt ? 'opacity-50' : ''}`}
          >
            <div className="flex items-baseline gap-2">
              <span className={`font-bold ${tx.type === 'charge' ? 'text-red-700' : 'text-green-700'}`}>
              ...
              {tx.currency !== 'USD' && (
                <span className="font-mono text-xs text-gray-500">≈ ${Number(tx.amountUsd).toFixed(2)}</span>
              )}
              {tx.method && (
            ...
            <div className="mt-0.5 flex items-baseline gap-2 text-xs text-gray-500">
              {batchCode && <span className="font-mono font-semibold">{batchCode}</span>}
              {tx.note && <span className="truncate">{tx.note}</span>}
              <span>{createdByName}</span>
              {tx.voidedAt ? (
                <span className="text-red-700">✖ {t('voided')}: {tx.voidReason}</span>
              ) : (
                canManage && (
