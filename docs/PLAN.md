...
**Phase 3 — driver tracking (owner's request, 2026-07-25) — ✅ SHIPPED:** trip-scoped pairing (`driver_devices`, migration 0018) and the **GSRDriver Android app** (`apps/driver-android/`, APK built by GitHub Actions — no store, works in China). One position every 2 h by default (1/2/3 h selectable), GPS off in between, offline queue, stops by itself when the batch closes. iPhone/HarmonyOS stay on the logist's manual pins. DECISIONS #118–120. **v1.3 (round 55, 2026-08-04):** the after-2-hours death fixed — schedule moved from an AlarmManager chain to a persisted JobScheduler job, service is per-cycle, no notification at all on Android 13+; server-side silent-truck alarm ('TruckSilent' → plans.manage, once per silence, FRESH_MINUTES threshold, migration 0057). DECISIONS #482–486. **Owner action: release the 1.3 APK** (Actions → driver-apk → artifact → Admin → Haydovchi ilovasi; phones must uninstall the old one first).
...
**Phase 2.3 — CRM (owner: "CRM ni boshla", 2026-07-25) — ✅ SHIPPED:** leads with a fully editable funnel (stages: add/rename/recolour/reorder/delete), editable sources, custom fields on lead AND client cards (nine types, validated), one contact history, a single follow-up list covering leads and clients, dormant-client report, person grouping for multiple codes per human being, and two per-recipient Telegram digests. Migrations 0021–0022, DECISIONS #129–132.
...
- **Phase 2.3 CRM — ✅ SHIPPED 2026-07-25** (DECISIONS #129–132): `/crm` call list, `/crm/leads` funnel board, lead card with stage mover + contact log + convert-to-client, `/crm/dormant`, `/crm/people`, `/crm/settings` (stages, sources, custom fields); CRM sections on the client card; `crm.leads` / `crm.leads.view_all` / `crm.manage`.
