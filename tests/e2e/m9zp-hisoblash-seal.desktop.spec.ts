...
test('a request opens with a workspace that refuses to price it', async ({ page }) => {
  ...
  // a name shared with another spec arrives with that spec's baza already
  // filled — and typing the same number is a no-op that leaves the save
  // button disabled for ever. A seal is CONFIGURATION for every later spec
  // about the same product (#183 in a new costume), and the fixture is where
  // that is answered.
...
test('type the code, save, baza, confirm — then seal (VED 2.0 table)', async ({ page }) => {
  expect(requestUrl).not.toBe('');
  await login(page, ADMIN);
  await page.goto(requestUrl);
  ...
  await expect(page.getByTestId('calc-ungrouped-head')).toHaveCount(0);
  // The dictionary's word arrives grey with the mint: 10 % / 12 %.
  await expect(page.getByTestId('calc-group-rates')).toContainText('10%');
