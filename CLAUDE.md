...
Latest migration: **0076** (`client_notices` — the claim ledger behind «one
...
releases it; count must reach **77**;
...
**HIS SERVER IS AT 76, AND THIS BRANCH TAKES IT TO 77.** He deployed
...
`docker compose run --rm migrate` → **77**. 0076 creates one empty table and
...
length (**77** with this branch, 76 on `main` today) — the client book, the stock table and `/o/<code>` read
...
warehouse by name, so a second hub just works. «ozbga kirdi» and «rastamojka»
are ONE rung, stated: nothing stamps a cleared declaration, and a stage that
advances only when a human remembers is a stage that lies. **The ETA needed no
...
Red-proofs ×6. 1449 unit/integration; 153/154 e2e (only the pre-existing
m9z-nav-progress). Screenshots at 360×800 in uz and ru, document 360 wide.
